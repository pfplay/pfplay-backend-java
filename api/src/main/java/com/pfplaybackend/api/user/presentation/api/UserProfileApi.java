...
public interface UserProfileApi {
    ...
    public ResponseEntity<?> setUserProfile(
            @Valid @RequestBody UpdateMyAvatarBodyRequest request
    );
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200",
                    ...
                    content = @Content(
                            schema = @Schema(implementation = OtherProfileSummaryResponse.class)
                    )
            ),
            @ApiResponse(responseCode = "500",
    ...
    ResponseEntity<?> getUserProfile(CustomAuthentication authentication);
}
